...
barplot(counts, main="P(PKA)", horiz = TRUE,
        xlab="probablity", col=c("black","lightgrey"),xlim=c(0,1),
        legend = c("without surprise","with surprise"), beside=TRUE)


warning("Ken has just stopped R execution!")
#quit()
stopifnot(stopme)

stopme <- TRUE
#if(stopme == TRUE) stop("Ken has just stopped R execution!")
